...
for seed in range(num_realisations):
    ...
    wt_sol = pd.read_csv(simulation_path + "/SHH_NEURAL_TUBE/shh_neural_tube_grn_network_ode_sol_" + str(seed) + ".csv")
    wt_sol_final = wt_sol[wt_sol['t'] == wt_sol['t'].max()]

    adata_cont = sc.AnnData(X=wt_sol_final[['Shh', 'Ptch1', 'Shh_bound', 'Nkx2-2', 'Olig2', 'Pax6', 'Irx3']].values,
                            var=pd.DataFrame(index=['Shh', 'Ptch1', 'Shh_bound', 'Nkx2-2', 'Olig2', 'Pax6', 'Irx3'],
                                             data={'type':['ligand', 'receptor', 'bound_complex', 'factor', 'factor', 'factor', 'factor']}))
    ...
    adata_cont.var_names = pd.Index(['Shh', 'Ptch1', 'Shh_bound', 'Nkx2-2', 'Olig2', 'Pax6', 'Irx3'])
    ...
    pert_sol = pd.read_csv(simulation_path + "/SHH_NEURAL_TUBE/shh_neural_tube_grn_network_ode_sol_pert_" + str(seed) + ".csv")
    pert_sol_final = pert_sol[pert_sol['t'] == pert_sol['t'].max()]

    adata_pert = sc.AnnData(X=pert_sol_final[['Shh', 'Ptch1', 'Shh_bound', 'Nkx2-2', 'Olig2', 'Pax6', 'Irx3']].values,
                            var=pd.DataFrame(index=['Shh', 'Ptch1', 'Shh_bound', 'Nkx2-2', 'Olig2', 'Pax6', 'Irx3'],
                                             data={'type':['ligand', 'receptor', 'bound_complex', 'factor', 'factor', 'factor', 'factor']}))    
    ...
    adata_pert.var_names = pd.Index(['Shh', 'Ptch1', 'Shh_bound', 'Nkx2-2', 'Olig2', 'Pax6', 'Irx3'])
...
adata.write(simulation_path + "/SHH_NEURAL_TUBE/adata_shh_neural_grn.h5ad", compression='gzip')
