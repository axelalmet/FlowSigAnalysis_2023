...
for seed in range(num_realisations):
    ...
    wt_sol = pd.read_csv(simulation_path + "/SHH_FOXF1_BMP4/simple_shh_foxf1_bmp4_grn_network_ode_sol_" + str(seed) + ".csv")
    wt_sol_final = wt_sol[wt_sol['t'] == wt_sol['t'].max()]

    adata_cont = sc.AnnData(X=wt_sol_final[['Shh', 'Ptch1', 'Shh_bound', 'Foxf1', 'Bmp4']].values,
                            var=pd.DataFrame(index=['Shh', 'Ptch1', 'Shh_bound', 'Foxf1', 'Bmp4'],
                                             data={'type':['ligand', 'receptor', 'bound_complex', 'factor', 'ligand']}))
    ...
    adata_cont.var_names = pd.Index(['Shh', 'Ptch1', 'Shh_bound', 'Foxf1', 'Bmp4'])
    ...
    pert_sol = pd.read_csv(simulation_path + "/SHH_FOXF1_BMP4/simple_shh_foxf1_bmp4_grn_network_ode_pert_sol_" + str(seed) + ".csv")
    pert_sol_final = pert_sol[pert_sol['t'] == pert_sol['t'].max()]

    adata_pert = sc.AnnData(X=pert_sol_final[['Shh', 'Ptch1', 'Shh_bound', 'Foxf1', 'Bmp4']].values,
                            var=pd.DataFrame(index=['Shh', 'Ptch1', 'Shh_bound', 'Foxf1', 'Bmp4'],
                                             data={'type':['ligand', 'receptor', 'bound_complex', 'factor', 'ligand']}))    
    ...
    adata_pert.var_names = pd.Index(['Shh', 'Ptch1', 'Shh_bound', 'Foxf1', 'Bmp4'])
...
adata.write(simulation_path + "/SHH_FOXF1_BMP4/adata_shh_foxf1_bmp4.h5ad", compression='gzip')
